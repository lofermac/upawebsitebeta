...
interface MultiCountrySelectProps {
  value: string[]; // Array de country codes
  onChange: (countryCodes: string[]) => void;
}

export default function MultiCountrySelect({ value, onChange }: MultiCountrySelectProps) {
    ...
    switch (e.key) {
      ...
      case 'Enter':
        ...
        if (filteredCountries[highlightedIndex]) {
          handleToggleCountry(filteredCountries[highlightedIndex]);
        }
        break;
      case 'Escape':
        e.preventDefault();
  ...
  const handleToggleCountry = (country: Country) => {
    if (value.includes(country.code)) {
      // Remove país
      onChange(value.filter(code => code !== country.code));
    } else {
      // Adiciona país
  ...
  // Get selected countries objects
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      <div className="flex flex-wrap gap-2 min-h-[44px] p-3 bg-gray-800/50 rounded-lg border border-gray-700 mb-2">
        {selectedCountries.length > 0 ? (
          selectedCountries.map((country) => (
            <span 
              key={country.code}
              className="inline-flex items-center gap-2 px-3 py-1.5 bg-green-500/10 
                       border border-green-500/30 rounded-full text-sm text-green-400"
            >
              <country.Flag className="w-5 h-3.5 rounded-sm shadow-sm" />
          ...
          <span className="text-gray-500 text-sm">Global availability (all countries)</span>
        )}
      </div>
      ...
      <div className="relative group/input">
        ...
        <div
          ...
          className={`relative w-full pl-12 pr-12 py-3 bg-white/5 border ${isOpen ? 'border-[#077124]/50 ring-2 ring-[#077124]/20' : 'border-white/10'} rounded-lg text-white cursor-pointer transition-all duration-300 hover:border-white/[0.12] hover:bg-white/[0.08] flex items-center gap-3 focus:outline-none focus:ring-2 focus:ring-[#077124]`}
        >
          {isOpen ? (
            <input
              ref={inputRef}
            ...
            <span className="text-gray-400 text-sm">
              {selectedCountries.length > 0 
                ...
                : 'Select countries (leave empty for global)'}
            </span>
          )}
        </div>
        ...
        {isOpen && (
          <div 
            ref={dropdownRef}
            className="absolute z-50 w-full mt-2 bg-[#121212] border border-white/10 rounded-xl shadow-2xl shadow-black/50 overflow-hidden"
            style={{
              maxHeight: '300px',
              overflowY: 'auto',
            }}
              ...
              filteredCountries.map((country, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    {isSelected && (
                      <svg className="w-5 h-5 text-[#077124]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      <p className="text-xs text-gray-500 mt-2">
        {selectedCountries.length > 0 
          ? `💡 Available in ${selectedCountries.length} ${selectedCountries.length === 1 ? 'country' : 'countries'}` 
          : "💡 Leave empty for global availability"}
      </p>
    </div>
  );
}
