...
interface SubAffiliateData {
  ...
  name: string;
  referrals: number;
  revenue: number;          // Net Rake (commission earned)
  grossRake: number;        // Gross Rake generated by referred players
}

interface TopPlayer {
  id: string;
  name: string;
  nickname: string;      // Platform username
  platform: string;
  earnings: number;      // Net rake (kept for compatibility)
  grossRake: number;     // Gross rake generated
}
...
function IntelligenceContent() {
  ...
  // ============================================
  const [subAffPeriod, setSubAffPeriod] = useState<'60d' | '90d' | '120d' | '180d' | 'all'>('all');
  const [subAffPlatform, setSubAffPlatform] = useState<'all' | number>('all');
  const [topSubAffiliates, setTopSubAffiliates] = useState<SubAffiliateData[]>([]);
  
  const [topPlayersPeriod, setTopPlayersPeriod] = useState<'60d' | '90d' | '120d' | '180d' | 'all'>('all');
  const [topPlayersPlatform, setTopPlayersPlatform] = useState<'all' | number>('all');
  const [topPlayers, setTopPlayers] = useState<TopPlayer[]>([]);
  
  const [isLoadingSubAff, setIsLoadingSubAff] = useState(true);
  const [isLoadingTopPlayers, setIsLoadingTopPlayers] = useState(true);
  const [isLoadingExtras, setIsLoadingExtras] = useState(true);

  // Modal states for Sub-Affiliates
  const [isSubAffModalOpen, setIsSubAffModalOpen] = useState(false);
  const [allSubAffiliates, setAllSubAffiliates] = useState<SubAffiliateData[]>([]);
  const [subAffModalPeriod, setSubAffModalPeriod] = useState<'60d' | '90d' | '120d' | '180d' | 'all'>('all');
  const [subAffModalPlatform, setSubAffModalPlatform] = useState<'all' | number>('all');
  const [subAffModalPage, setSubAffModalPage] = useState(1);
  const [isLoadingSubAffModal, setIsLoadingSubAffModal] = useState(false);

  // Modal states for Top Players
  const [isPlayersModalOpen, setIsPlayersModalOpen] = useState(false);
  const [allPlayers, setAllPlayers] = useState<TopPlayer[]>([]);
  const [playersModalPeriod, setPlayersModalPeriod] = useState<'60d' | '90d' | '120d' | '180d' | 'all'>('all');
  const [playersModalPlatform, setPlayersModalPlatform] = useState<'all' | number>('all');
  const [playersModalPage, setPlayersModalPage] = useState(1);
  const [isLoadingPlayersModal, setIsLoadingPlayersModal] = useState(false);
  ...
  // ============================================
  // 6a. LOAD TOP SUB-AFFILIATES (with filters)
  // ============================================
  useEffect(() => {
    async function loadSubAffiliates() {
      setIsLoadingSubAff(true);
      try {
        // Get date range based on period
        const { start, end } = getDateRange(subAffPeriod);
        const startYear = start.getFullYear();
        const startMonth = start.getMonth() + 1;
        const endYear = end.getFullYear();
        const endMonth = end.getMonth() + 1;

        // Fetch all sub-affiliates
        ...
        const subAffiliatesWithData = await Promise.all(
          (subAffiliates || []).map(async (sa: SubAffiliateQuery) => {
            // Get all referrals for this sub-affiliate
            ...
            // Query earnings with period AND platform filters
            let earningsQuery = supabase
              .from('player_earnings')
              .select('net_rake, gross_rake, period_month, period_year, player_deal_id, player_deals!inner(deal_id)')
              .in('player_deal_id', playerDealIds);
            ...
            const { data: earningsRaw } = await earningsQuery;
            
            // Filter by exact period in JavaScript
            let filteredEarnings = earningsRaw || [];
            if (subAffPeriod !== 'all') {
              filteredEarnings = (earningsRaw || []).filter((e: unknown) => {
                const earning = e as { period_year: number; period_month: number };
                const periodNum = earning.period_year * 100 + earning.period_month;
                const startNum = startYear * 100 + startMonth;
                const endNum = endYear * 100 + endMonth;
                return periodNum >= startNum && periodNum <= endNum;
            ...
            if (subAffPlatform !== 'all') {
              filteredEarnings = filteredEarnings.filter((e: unknown) => {
                const earning = e as { player_deals?: { deal_id: number } };
                return earning.player_deals?.deal_id === subAffPlatform;
              });
            }

            const revenue = filteredEarnings.reduce(
              (sum, e: unknown) => {
                const earning = e as { net_rake: string | number };
                return sum + (parseFloat(String(earning.net_rake)) || 0);
              }, 
              0
            );

            const grossRake = filteredEarnings.reduce(
              (sum, e: unknown) => {
                const earning = e as { gross_rake: string | number };
                return sum + (parseFloat(String(earning.gross_rake)) || 0);
              }, 
              0
            );
        ...
        // Sort by Gross Rake and take top 5
        const topSubAffiliatesData: SubAffiliateData[] = subAffiliatesWithData
          .sort((a, b) => b.grossRake - a.grossRake)
          .slice(0, 5)
          .map(sa => ({
            id: sa.id,
            referral_code: sa.referral_code,
        ...
        setIsLoadingSubAff(false);
      }
    }

    loadSubAffiliates();
  }, [subAffPeriod, subAffPlatform]);

  // ============================================
  // 6b. LOAD TOP RAKING PLAYERS (with filters, ordered by Gross Rake)
  // ============================================
  useEffect(() => {
    async function loadTopPlayers() {
      ...
      try {
        // Get date range based on period
        const { start, end } = getDateRange(topPlayersPeriod);
        const startYear = start.getFullYear();
        const startMonth = start.getMonth() + 1;
        const endYear = end.getFullYear();
        const endMonth = end.getMonth() + 1;

        // Query earnings with period filter
        let earningsQuery = supabase
          .from('player_earnings')
          .select('gross_rake, net_rake, period_month, period_year, player_deal_id, player_deals!inner(deal_id, platform_username, deals!inner(name))');

        // Apply period filter
        if (topPlayersPeriod !== 'all') {
          earningsQuery = earningsQuery
            .gte('period_year', startYear)
            .lte('period_year', endYear);
        }
        ...
        // Apply platform filter
        if (topPlayersPlatform !== 'all') {
          filteredEarnings = filteredEarnings.filter((e: unknown) => {
            const earning = e as { player_deals?: { deal_id: number } };
            return earning.player_deals?.deal_id === topPlayersPlatform;
          });
        }

        // Group by player_deal_id and calculate totals
        const playerDealMap: Record<string, {
          nickname: string;
          platform: string;
          grossRake: number;
          netRake: number;
        ...
        filteredEarnings.forEach((e: unknown) => {
          const earning = e as {
            player_deal_id: string;
            gross_rake: string | number;
            net_rake: string | number;
          ...
          const playerDealId = earning.player_deal_id;
          const nickname = earning.player_deals?.platform_username || 'Unknown';
          const platform = earning.player_deals?.deals?.name || 'Unknown';
          const grossRake = parseFloat(String(earning.gross_rake)) || 0;
          const netRake = parseFloat(String(earning.net_rake)) || 0;

          if (!playerDealMap[playerDealId]) {
            playerDealMap[playerDealId] = {
              nickname,
              platform,
              grossRake: 0,
              netRake: 0
          ...
          playerDealMap[playerDealId].grossRake += grossRake;
          playerDealMap[playerDealId].netRake += netRake;
        });

        // Convert to array and sort by Gross Rake
        const topPlayersData: TopPlayer[] = Object.entries(playerDealMap)
          .map(([id, data]) => ({
            id,
            name: data.nickname, // Will be overwritten by nickname
            nickname: data.nickname,
            platform: data.platform,
            earnings: data.netRake,
            grossRake: data.grossRake
          }))
          .sort((a, b) => b.grossRake - a.grossRake)
          .slice(0, 5);
    ...
    loadTopPlayers();
  }, [topPlayersPeriod, topPlayersPlatform]);

  // ============================================
  // 6c. LOAD ALL SUB-AFFILIATES FOR MODAL
  // ============================================
  useEffect(() => {
    if (!isSubAffModalOpen) return;

    async function loadAllSubAffiliates() {
      setIsLoadingSubAffModal(true);
      try {
        const { start, end } = getDateRange(subAffModalPeriod);
        const startYear = start.getFullYear();
        const startMonth = start.getMonth() + 1;
        const endYear = end.getFullYear();
        const endMonth = end.getMonth() + 1;
        ...
        const subAffiliatesWithData = await Promise.all(
          (subAffiliates || []).map(async (sa: SubAffiliateQuery) => {
            ...
            if (subAffModalPlatform !== 'all') {
              filteredEarnings = filteredEarnings.filter((e: unknown) => {
                const earning = e as { player_deals?: { deal_id: number } };
                return earning.player_deals?.deal_id === subAffModalPlatform;
              });
            }
        ...
        const allSubAffiliatesData: SubAffiliateData[] = subAffiliatesWithData
          .sort((a, b) => b.grossRake - a.grossRake)
          .map(sa => ({
            id: sa.id,
            referral_code: sa.referral_code,
        ...
        setAllSubAffiliates(allSubAffiliatesData);
        setSubAffModalPage(1); // Reset to first page
      } catch (error) {
        console.error('Error loading all sub-affiliates:', error);
        setAllSubAffiliates([]);
      } finally {
        setIsLoadingSubAffModal(false);
      }
    }

    loadAllSubAffiliates();
  }, [isSubAffModalOpen, subAffModalPeriod, subAffModalPlatform]);

  // ============================================
  // 6d. LOAD ALL PLAYERS FOR MODAL
  // ============================================
  useEffect(() => {
    if (!isPlayersModalOpen) return;

    async function loadAllPlayers() {
      setIsLoadingPlayersModal(true);
      try {
        const { start, end } = getDateRange(playersModalPeriod);
        const startYear = start.getFullYear();
        const startMonth = start.getMonth() + 1;
        const endYear = end.getFullYear();
        const endMonth = end.getMonth() + 1;
        ...
        if (playersModalPlatform !== 'all') {
          filteredEarnings = filteredEarnings.filter((e: unknown) => {
            const earning = e as { player_deals?: { deal_id: number } };
            return earning.player_deals?.deal_id === playersModalPlatform;
          });
        }
        ...
        const allPlayersData: TopPlayer[] = Object.entries(playerDealMap)
          .map(([id, data]) => ({
            id,
            name: data.nickname,
            nickname: data.nickname,
        ...
        setPlayersModalPage(1); // Reset to first page
      } catch (error) {
        console.error('Error loading all players:', error);
        setAllPlayers([]);
      } finally {
        setIsLoadingPlayersModal(false);
      }
    }

    loadAllPlayers();
  }, [isPlayersModalOpen, playersModalPeriod, playersModalPlatform]);
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-[#0a0e13] border border-gray-800 rounded-lg p-6">
          {/* Header with Filters */}
          <div className="mb-4">
            <h3 className="text-lg font-bold text-white mb-3">Top Sub-Affiliates</h3>
            
            {/* Period Filter Buttons - Same style as Revenue Trend */}
            <div className="flex flex-wrap gap-2 mb-3">
              <button
                onClick={() => setSubAffPeriod('60d')}
                className={`px-3 py-1.5 text-sm rounded-lg transition-colors ${
                  subAffPeriod === '60d'
                    ? 'bg-emerald-600 text-white'
                    : 'bg-[#1a1f2e] text-gray-400 hover:bg-[#242938]'
                }`}
              >
                Last 60 Days
              </button>
              <button
                onClick={() => setSubAffPeriod('90d')}
                className={`px-3 py-1.5 text-sm rounded-lg transition-colors ${
                  subAffPeriod === '90d'
                    ? 'bg-emerald-600 text-white'
                    : 'bg-[#1a1f2e] text-gray-400 hover:bg-[#242938]'
            ...
            {/* Platform Dropdown - Same style as Revenue Trend */}
            <select
              value={subAffPlatform}
              onChange={(e) => setSubAffPlatform(e.target.value === 'all' ? 'all' : Number(e.target.value))}
              className="w-full bg-[#1a1f2e] border border-gray-700 rounded-lg px-4 py-2 text-white text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent"
            >
              <option value="all">All Platforms</option>
              {availableDeals.map(deal => (
                <option key={deal.id} value={deal.id}>
          ...
          {isLoadingSubAff ? (
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between px-3 pb-2 border-b border-gray-700">
                ...
                <span className="text-xs font-semibold text-gray-400 uppercase">Total Gross Rake</span>
              </div>
              ...
              {topSubAffiliates.map((affiliate, index) => (
                <div key={affiliate.id} className="flex items-center justify-between p-3 bg-gray-900/50 rounded-lg hover:bg-gray-900/70 transition-colors">
                  ...
                  <span className="text-sm font-bold text-emerald-500">
                    ${affiliate.grossRake.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </span>
                </div>
              ))}
            </div>
          ...
          {/* View More Button */}
          {!isLoadingSubAff && topSubAffiliates.length > 0 && (
            <button
              onClick={() => setIsSubAffModalOpen(true)}
              className="w-full mt-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-medium rounded-lg transition-colors"
            >
              View More
            </button>
          )}
        ...
        <div className="bg-[#0a0e13] border border-gray-800 rounded-lg p-6">
          {/* Header with Filters */}
          <div className="mb-4">
            <h3 className="text-lg font-bold text-white mb-3">Top Raking Players</h3>
            
            {/* Period Filter Buttons - Same style as Revenue Trend */}
            <div className="flex flex-wrap gap-2 mb-3">
              <button
                onClick={() => setTopPlayersPeriod('60d')}
                className={`px-3 py-1.5 text-sm rounded-lg transition-colors ${
                  topPlayersPeriod === '60d'
                    ? 'bg-emerald-600 text-white'
                    : 'bg-[#1a1f2e] text-gray-400 hover:bg-[#242938]'
                }`}
              >
              ...
              <button
                onClick={() => setTopPlayersPeriod('90d')}
                className={`px-3 py-1.5 text-sm rounded-lg transition-colors ${
                  topPlayersPeriod === '90d'
                    ? 'bg-emerald-600 text-white'
                    : 'bg-[#1a1f2e] text-gray-400 hover:bg-[#242938]'
                }`}
              >
              ...
              <button
                onClick={() => setTopPlayersPeriod('120d')}
                className={`px-3 py-1.5 text-sm rounded-lg transition-colors ${
                  topPlayersPeriod === '120d'
                    ? 'bg-emerald-600 text-white'
                    : 'bg-[#1a1f2e] text-gray-400 hover:bg-[#242938]'
                }`}
              >
              ...
              <button
                onClick={() => setTopPlayersPeriod('180d')}
                className={`px-3 py-1.5 text-sm rounded-lg transition-colors ${
                  topPlayersPeriod === '180d'
                    ? 'bg-emerald-600 text-white'
                    : 'bg-[#1a1f2e] text-gray-400 hover:bg-[#242938]'
                }`}
              >
              ...
              <button
                onClick={() => setTopPlayersPeriod('all')}
                className={`px-3 py-1.5 text-sm rounded-lg transition-colors ${
                  topPlayersPeriod === 'all'
                    ? 'bg-emerald-600 text-white'
                    : 'bg-[#1a1f2e] text-gray-400 hover:bg-[#242938]'
                }`}
              >
            ...
            {/* Platform Dropdown - Same style as Revenue Trend */}
            <select
              value={topPlayersPlatform}
              onChange={(e) => setTopPlayersPlatform(e.target.value === 'all' ? 'all' : Number(e.target.value))}
              className="w-full bg-[#1a1f2e] border border-gray-700 rounded-lg px-4 py-2 text-white text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent"
            >
              <option value="all">All Platforms</option>
              {availableDeals.map(deal => (
              ...
              {topPlayers.map((player, index) => (
                <div key={player.id} className="flex items-center justify-between p-3 bg-gray-900/50 rounded-lg hover:bg-gray-900/70 transition-colors">
                  ...
                  <span className="text-sm font-bold text-blue-400">
                    ${player.grossRake.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </span>
                </div>
              ))}
            </div>
          ...
          {/* View More Button */}
          {!isLoadingTopPlayers && topPlayers.length > 0 && (
            <button
              onClick={() => setIsPlayersModalOpen(true)}
              className="w-full mt-4 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg transition-colors"
            >
              View More
            </button>
          )}
      ...
      {/* Sub-Affiliates Modal */}
      {isSubAffModalOpen && (
        <div 
          className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4"
          onClick={() => setIsSubAffModalOpen(false)}
        >
          <div 
            className="bg-[#0a0e13] border border-gray-800 rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Modal Header */}
            <div className="border-b border-gray-800 p-6">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-2xl font-bold text-white">All Sub-Affiliates</h2>
                <button
                  onClick={() => setIsSubAffModalOpen(false)}
                  className="p-2 hover:bg-gray-800 rounded-lg transition-colors"
                >
              ...
              <div className="space-y-3">
                <div className="flex flex-wrap gap-2">
                  <button
                    onClick={() => setSubAffModalPeriod('60d')}
                    className={`px-3 py-1.5 text-sm rounded-lg transition-colors ${
                      subAffModalPeriod === '60d'
                        ? 'bg-emerald-600 text-white'
                        : 'bg-[#1a1f2e] text-gray-400 hover:bg-[#242938]'
                    }`}
                  >
                  ...
                  <button
                    onClick={() => setSubAffModalPeriod('90d')}
                    className={`px-3 py-1.5 text-sm rounded-lg transition-colors ${
                      subAffModalPeriod === '90d'
                        ? 'bg-emerald-600 text-white'
                        : 'bg-[#1a1f2e] text-gray-400 hover:bg-[#242938]'
                    }`}
                  >
                  ...
                  <button
                    onClick={() => setSubAffModalPeriod('120d')}
                    className={`px-3 py-1.5 text-sm rounded-lg transition-colors ${
                      subAffModalPeriod === '120d'
                        ? 'bg-emerald-600 text-white'
                        : 'bg-[#1a1f2e] text-gray-400 hover:bg-[#242938]'
                    }`}
                  >
                  ...
                  <button
                    onClick={() => setSubAffModalPeriod('180d')}
                    className={`px-3 py-1.5 text-sm rounded-lg transition-colors ${
                      subAffModalPeriod === '180d'
                        ? 'bg-emerald-600 text-white'
                        : 'bg-[#1a1f2e] text-gray-400 hover:bg-[#242938]'
                    }`}
                  >
                  ...
                  <button
                    onClick={() => setSubAffModalPeriod('all')}
                    className={`px-3 py-1.5 text-sm rounded-lg transition-colors ${
                      subAffModalPeriod === 'all'
                        ? 'bg-emerald-600 text-white'
                        : 'bg-[#1a1f2e] text-gray-400 hover:bg-[#242938]'
                    }`}
                  >
                ...
                <select
                  value={subAffModalPlatform}
                  onChange={(e) => setSubAffModalPlatform(e.target.value === 'all' ? 'all' : Number(e.target.value))}
                  className="w-full bg-[#1a1f2e] border border-gray-700 rounded-lg px-4 py-2 text-white text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent"
                >
                  <option value="all">All Platforms</option>
                  {availableDeals.map(deal => (
            ...
            <div className="flex-1 overflow-y-auto">
              {isLoadingSubAffModal ? (
                <div className="flex items-center justify-center h-40 text-gray-500">
                  <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-emerald-500"></div>
                </div>
              ) : allSubAffiliates.length > 0 ? (
                <>
                  {/* Table Headers - Sticky */}
                  <div className="flex items-center justify-between px-9 py-3 border-b border-gray-700 sticky top-0 bg-[#0a0e13] z-10">
                    <span className="text-xs font-semibold text-gray-400 uppercase">Name</span>
                    <span className="text-xs font-semibold text-gray-400 uppercase">Total Gross Rake</span>
                  </div>
                  ...
                  <div className="p-6 pt-3 space-y-3">
                    {allSubAffiliates
                      .slice((subAffModalPage - 1) * ITEMS_PER_PAGE, subAffModalPage * ITEMS_PER_PAGE)
                      .map((affiliate, index) => {
                        const globalIndex = (subAffModalPage - 1) * ITEMS_PER_PAGE + index;
                        return (
                          <div key={affiliate.id} className="flex items-center justify-between p-3 bg-gray-900/50 rounded-lg hover:bg-gray-900/70 transition-colors">
                            <div className="flex items-center gap-3">
                              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${
                                globalIndex === 0 ? 'bg-yellow-500 text-gray-900' :
                                globalIndex === 1 ? 'bg-gray-400 text-gray-900' :
                ...
                <div className="flex items-center justify-center h-40 text-gray-500 p-6">
                  No sub-affiliate data available
                </div>
              )}
            </div>
            ...
            {!isLoadingSubAffModal && allSubAffiliates.length > 0 && (
              <div className="border-t border-gray-800 p-4">
                <div className="flex items-center justify-between">
                  <div className="text-sm text-gray-400">
                    Showing {((subAffModalPage - 1) * ITEMS_PER_PAGE) + 1} to {Math.min(subAffModalPage * ITEMS_PER_PAGE, allSubAffiliates.length)} of {allSubAffiliates.length} sub-affiliates
                  </div>
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() => setSubAffModalPage(p => Math.max(1, p - 1))}
                      disabled={subAffModalPage === 1}
                      className="px-4 py-2 bg-[#1a1f2e] hover:bg-[#242938] text-white rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      Previous
                    </button>
                    <span className="text-sm text-gray-400">
                      Page {subAffModalPage} of {Math.ceil(allSubAffiliates.length / ITEMS_PER_PAGE)}
                    </span>
                    <button
                      onClick={() => setSubAffModalPage(p => Math.min(Math.ceil(allSubAffiliates.length / ITEMS_PER_PAGE), p + 1))}
                      disabled={subAffModalPage >= Math.ceil(allSubAffiliates.length / ITEMS_PER_PAGE)}
                      className="px-4 py-2 bg-[#1a1f2e] hover:bg-[#242938] text-white rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      Next
                    </button>
      ...
      {isPlayersModalOpen && (
        ...
        >
          ...
          >
            ...
            <div className="border-b border-gray-800 p-6">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-2xl font-bold text-white">All Raking Players</h2>
                <button
                  onClick={() => setIsPlayersModalOpen(false)}
                  className="p-2 hover:bg-gray-800 rounded-lg transition-colors"
                >
              ...
              <div className="space-y-3">
                <div className="flex flex-wrap gap-2">
                  <button
                    onClick={() => setPlayersModalPeriod('60d')}
                    className={`px-3 py-1.5 text-sm rounded-lg transition-colors ${
                      playersModalPeriod === '60d'
                        ? 'bg-emerald-600 text-white'
                        : 'bg-[#1a1f2e] text-gray-400 hover:bg-[#242938]'
                    }`}
                  >
                  ...
                  <button
                    onClick={() => setPlayersModalPeriod('90d')}
                    className={`px-3 py-1.5 text-sm rounded-lg transition-colors ${
                      playersModalPeriod === '90d'
                        ? 'bg-emerald-600 text-white'
                        : 'bg-[#1a1f2e] text-gray-400 hover:bg-[#242938]'
                    }`}
                  >
                  ...
                  <button
                    onClick={() => setPlayersModalPeriod('120d')}
                    className={`px-3 py-1.5 text-sm rounded-lg transition-colors ${
                      playersModalPeriod === '120d'
                        ? 'bg-emerald-600 text-white'
                        : 'bg-[#1a1f2e] text-gray-400 hover:bg-[#242938]'
                    }`}
                  >
                  ...
                  <button
                    onClick={() => setPlayersModalPeriod('180d')}
                    className={`px-3 py-1.5 text-sm rounded-lg transition-colors ${
                      playersModalPeriod === '180d'
                        ? 'bg-emerald-600 text-white'
                        : 'bg-[#1a1f2e] text-gray-400 hover:bg-[#242938]'
                    }`}
                  >
                  ...
                  <button
                    onClick={() => setPlayersModalPeriod('all')}
                    className={`px-3 py-1.5 text-sm rounded-lg transition-colors ${
                      playersModalPeriod === 'all'
                        ? 'bg-emerald-600 text-white'
                        : 'bg-[#1a1f2e] text-gray-400 hover:bg-[#242938]'
                    }`}
                  >
                ...
                <select
                  value={playersModalPlatform}
                  onChange={(e) => setPlayersModalPlatform(e.target.value === 'all' ? 'all' : Number(e.target.value))}
                  className="w-full bg-[#1a1f2e] border border-gray-700 rounded-lg px-4 py-2 text-white text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent"
                >
                  <option value="all">All Platforms</option>
                  {availableDeals.map(deal => (
            ...
            <div className="flex-1 overflow-y-auto">
              {isLoadingPlayersModal ? (
                ...
                <>
                  ...
                  <div className="p-6 pt-3 space-y-3">
                    {allPlayers
                      .slice((playersModalPage - 1) * ITEMS_PER_PAGE, playersModalPage * ITEMS_PER_PAGE)
                      .map((player, index) => {
                        const globalIndex = (playersModalPage - 1) * ITEMS_PER_PAGE + index;
                        return (
                          <div key={player.id} className="flex items-center justify-between p-3 bg-gray-900/50 rounded-lg hover:bg-gray-900/70 transition-colors">
                            <div className="flex items-center gap-3">
                              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${
            ...
            {/* Modal Footer with Pagination */}
            {!isLoadingPlayersModal && allPlayers.length > 0 && (
              <div className="border-t border-gray-800 p-4">
                <div className="flex items-center justify-between">
                  <div className="text-sm text-gray-400">
                    Showing {((playersModalPage - 1) * ITEMS_PER_PAGE) + 1} to {Math.min(playersModalPage * ITEMS_PER_PAGE, allPlayers.length)} of {allPlayers.length} players
                  </div>
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() => setPlayersModalPage(p => Math.max(1, p - 1))}
                      disabled={playersModalPage === 1}
                      className="px-4 py-2 bg-[#1a1f2e] hover:bg-[#242938] text-white rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      Previous
                    </button>
                    <span className="text-sm text-gray-400">
                      Page {playersModalPage} of {Math.ceil(allPlayers.length / ITEMS_PER_PAGE)}
                    </span>
                    <button
                      onClick={() => setPlayersModalPage(p => Math.min(Math.ceil(allPlayers.length / ITEMS_PER_PAGE), p + 1))}
                      disabled={playersModalPage >= Math.ceil(allPlayers.length / ITEMS_PER_PAGE)}
                      className="px-4 py-2 bg-[#1a1f2e] hover:bg-[#242938] text-white rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      Next
                    </button>
